...
from coffea.analysis_tools import PackedSelection
from src.utils import getZptWgts, getZptWgts_2016postVFP, merge_zpt_wgt, getRapidity, _mass2_kernel, p4_sum_mass, p4_subtract_pt, testJetVector, delta_r_V1, etaFrame_variables, cs_variables, getCosThetaCS, getPhiCS
import logging
...
class EventProcessor(processor.ProcessorABC):
    ...
    def process(self, events: coffea_nanoevent):
        ...
        events["Muon", "dxy_raw"] = ak.ones_like(events.Muon.dxy) * events.Muon.dxy
        events["Muon", "dz_raw"] = ak.ones_like(events.Muon.dz) * events.Muon.dz
        events["Muon", "tkRelIso_raw"] = ak.ones_like(events.Muon.tkRelIso) * events.Muon.tkRelIso
        events["Muon", "highPtId_raw"] = ak.ones_like(events.Muon.highPtId) * events.Muon.highPtId
        ...
        muon_selection = (
            ...
            & (abs(events.Muon.dxy_raw) < self.config["muon_dxy_cut"])
            & (abs(events.Muon.dz_raw) < self.config["muon_dz_cut"])
            & (events.Muon.tkRelIso_raw < self.config["muon_tkRelIso_cut"])
            & (events.Muon.highPtId_raw == self.config["muon_highPtId_cut"])
        ...
        self.selection.add("muon_dxy", ak.any(abs(events.Muon.dxy_raw) <= self.config["muon_dxy_cut"], axis=1))
        self.selection.add("muon_dz", ak.any(abs(events.Muon.dz_raw) <= self.config["muon_dz_cut"], axis=1))
        self.selection.add("muon_tkRelIso", ak.any(events.Muon.tkRelIso_raw <= self.config["muon_tkRelIso_cut"], axis=1))
        self.selection.add("muon_highPtId", ak.any(events.Muon.highPtId_raw == self.config["muon_highPtId_cut"], axis=1))
...
# --------------------------------------------------------
        ...
        if do_jec: # old method
            ...
            # -------------------------------------
            # jets = do_jec_scale(jets, self.config, is_mc, dataset) # FIXME, src/corrections/jet.py do_jet_scale()
            # if is_mc: # JER smearing # FIXME
...
# just reading part 2 start -------------------------
    ...
    def get_mass_resolution(self, dimuon, mu1,mu2, is_mc:bool, doing_BS_correction=False, test_mode=False):
        ...
        if "2016" in year: # 2016PreVFP, 2016PostVFP, 2016_RERECO
            ...
            yearUL = year
        doing_BS_correction = False # FIXME, no calib files for Run 3, need to know how to make or find them
    ...
    def jet_loop(
        ...
        if NanoAODv == 9: # FIXME: should update Jet_PUID for Nano12
        ...
        # jetHorn_puid_cut = (jets.puId >= 7) | (jets.pt >= 50) # tight pu Id # FIXME
        ...
        jet_loop_out_dict = {
            ...
            # f"jet1_puId_{variation}" : jet1.puId, # FIXME
            ...
            # f"jet1_pt_jec_{variation}" : jet1.pt_jec, # FIXME
            # f"jet2_pt_jec_{variation}" : jet2.pt_jec, # FIXME
            # f"jet1_mass_jec_{variation}" : jet1.mass_jec, # FIXME
            # f"jet2_mass_jec_{variation}" : jet2.mass_jec, # FIXME
            #-------------------------
            ...
            # f"jet2_puId_{variation}" : jet2.puId, # FIXME
